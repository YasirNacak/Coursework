...
void takeUserInput(int &boardSize, char gameField[20][20], int &currentPlayer, bool &playerVsAI, int &gameState);

bool checkWin(int boardSize, char gameField[20][20]);

bool checkDraw(int boardSize, char gameField[20][20]);

void vsComputerCalcMoveVal(int pointX, int pointY, char gameField[20][20], int boardSize, int &maxValueCurrentMove, int xIncrease = 1, int yIncrease = 1);

void vsComputerPlayMove(int boardSize, char gameField[20][20]);

void saveGame(const string &filename, int &gameState, bool &playerVsAI, int &currentPlayer, char gameField[20][20], int &boardSize);

bool loadGame(const string &filename, int &gameState, bool &playerVsAI, int &currentPlayer, char gameField[20][20], int &boardSize);
...
int main(){
    ...
    while(isRunning){
        ...
        if(gameState == 0){             //Game State 0 = Intro and board size input
            ...
            takeUserInput(boardSize, gameField, currentPlayer, playerVsAI, gameState);
            ...
            cout << "Game Over!" << endl;
            if(winningPlayer != 0){
                if(playerVsAI && winningPlayer == 2)
                    cout << "The Winner of this game is Computer!" << endl;        
                else
...
void takeUserInput(int &boardSize, char gameField[20][20], int &currentPlayer, bool &playerVsAI, int &gameState){
    ...
    while(true){
        ...
        if(inputStr == "SAVE"){
    		cin >> filename;
    		saveGame(filename, gameState, playerVsAI, currentPlayer, gameField, boardSize);
    	} else if(inputStr == "LOAD"){
    		cin >> filename;
    		oldPlayervsAI = playerVsAI;
    		if(loadGame(filename, gameState, playerVsAI, currentPlayer, gameField, boardSize)){
    			cout << "Game Loaded" << endl;
    			drawGameField(boardSize, gameField);
    			//Error fixage
    			if(oldPlayervsAI == 0 && playerVsAI == 1)
    				break;
    		}
...
void vsComputerCalcMoveVal(int pointX, int pointY, char gameField[20][20], int boardSize, int &maxValueCurrentMove, int xIncrease, int yIncrease){
    int i, result = 0;
    for(i=0; i<4; i++){
...
void vsComputerPlayMove(int boardSize, char gameField[20][20]){
    ...
    for(j=0; j<boardSize; j++){
        ...
        if(i!=0){
            ...
            vsComputerCalcMoveVal(i-1, j-3, gameField, boardSize, maxValueCurrentMove, 0);
            vsComputerCalcMoveVal(i-1, j-2, gameField, boardSize, maxValueCurrentMove, 0);
            vsComputerCalcMoveVal(i-1, j-1, gameField, boardSize, maxValueCurrentMove, 0);
            vsComputerCalcMoveVal(i-1, j+0, gameField, boardSize, maxValueCurrentMove, 0);

            //Vertical effectiveness of the move
            vsComputerCalcMoveVal(i-4, j+0, gameField, boardSize, maxValueCurrentMove, 1, 0);
            vsComputerCalcMoveVal(i-3, j+0, gameField, boardSize, maxValueCurrentMove, 1, 0);
            vsComputerCalcMoveVal(i-2, j+0, gameField, boardSize, maxValueCurrentMove, 1, 0);
            vsComputerCalcMoveVal(i-1, j+0, gameField, boardSize, maxValueCurrentMove, 1, 0);

            //Diagonal from left to right effectiveness of the move
            vsComputerCalcMoveVal(i-4, j-3, gameField, boardSize, maxValueCurrentMove);
            vsComputerCalcMoveVal(i-3, j-2, gameField, boardSize, maxValueCurrentMove);
            vsComputerCalcMoveVal(i-2, j-1, gameField, boardSize, maxValueCurrentMove);
            vsComputerCalcMoveVal(i-1, j+0, gameField, boardSize, maxValueCurrentMove);

            //Diagonal from right to left effectiveness of the move
            vsComputerCalcMoveVal(i-4, j+3, gameField, boardSize, maxValueCurrentMove, 1, -1);
            vsComputerCalcMoveVal(i-3, j+2, gameField, boardSize, maxValueCurrentMove, 1, -1);
            vsComputerCalcMoveVal(i-2, j+1, gameField, boardSize, maxValueCurrentMove, 1, -1);
            vsComputerCalcMoveVal(i-1, j+0, gameField, boardSize, maxValueCurrentMove, 1, -1);
...
void saveGame(const string &filename, int &gameState, bool &playerVsAI, int &currentPlayer, char gameField[20][20], int &boardSize){
	...
	gameFile << gameState << " " << playerVsAI << " " << currentPlayer << " " << boardSize << endl;
	for(int i=0; i<boardSize; i++){
		for(int j=0; j<boardSize; j++)
			gameFile << gameField[i][j] << " ";
		gameFile << endl;
	}
	gameFile.close();
}
...
bool loadGame(const string &filename, int &gameState, bool &playerVsAI, int &currentPlayer, char gameField[20][20], int &boardSize){
	...
	if(gameFile){
		result = true;
		gameFile >> gameState >> playerVsAI >> currentPlayer >> boardSize;
		for(int i=0; i<boardSize; i++)
			for(int j=0; j<boardSize; j++)
				gameFile >> gameField[i][j];
	} else {
