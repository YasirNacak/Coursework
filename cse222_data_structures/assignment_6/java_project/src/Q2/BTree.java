...
public class BTree<E extends Comparable<E>> implements SearchTree<E> {
    ...
    private boolean insert(Node<E> root, E item) {
        ...
        //System.out.println("index = " + index + " data size = " + root.size);
        if (index != root.size) {
            if (item.compareTo(root.data[index]) == 0)
                return false;
        }
    ...
    /**
     * Binary Seearch Method, additional return values are determined by the requirements
     * of the insert method of BTree
     *
     * @param target value that needs to be searched
     * @param items  list of items that the search is being done in
     * @param first  lower bound of the binary search
     * @param last   upper bound of the binary search
     * @return if the target is in the items, the position of the item, otherwise the spot that target needs to go in items
     */
    public int binarySearch(E target, E[] items, int first, int last) {
        System.out.println("binary search is being done");
        // case : target is bigger than the last value of the array
        ...
        // case : target is not in the array, we need to find the spot it needs to go
