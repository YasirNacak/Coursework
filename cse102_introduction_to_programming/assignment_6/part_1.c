...
int openCell(cell grid[][GRIDSIZE], int x, int y){
    ...
    if(isLocationLegal(x,y)){   /*first checks if the given location is legal*/
        for(i=0; i<3; i++){ /*double for's is for checking every grid between the given one*/
            xAway = -1;
            for(j=0; j<3; j++){
                if(isLocationLegal(x+xAway, y+yAway) && grid[x+xAway][y+yAway] == closedEmpty && xAway+yAway!=0)   /*checks if the given cell is legal and closed*/
                    openCell(grid, x+xAway, y+yAway);
                xAway++;
            }
            yAway++;
        }
...
int asMain(){
    ...
    int totalEmptyCell, totalMinedCell, openEmptyCell, flaggedMinedCell;
    int gameState;
    ...
    */
    ...
    /*
        ...
        and mined cells which is going to be used later
        for checking if the player win.
    */
    ...
    while(gameState == 0){
        ...
        if(openEmptyCell == totalEmptyCell || flaggedMinedCell == totalMinedCell){
            gameState = 1;
        }
    }
    if(gameState == 1)
        printf("you win the game in %d moves.", moveCount);
